...
class UrlTree<T> {
  ...
  void printBFS() {
    ...
    while (queue.isNotEmpty) {
      ...
      for (final _TreeNode<T> child in temp.children.values) {
        queue.add(child);
      }
      debugPrint('$temp(${temp.urlPart}) --> ${temp.value}');
    }
  }
  ...
  T? find(String url) {
    final String cleanedUrl = _cleanUrl(url);
    final node = _find(root, cleanedUrl.split('/'));
    return node?.value;
  }
  ...
  _TreeNode? _find(_TreeNode node, List<String> parts) {
    ...
    if (exactMatchNode != null) {
      ...
      for (final _TreeNode<T?> childNode
